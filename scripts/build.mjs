...
const srtToVttTool = toolsData.tools.find(tool => tool.id === 'media-srt-to-vtt');
if (!srtToVttTool) throw new Error('Falta la herramienta media-srt-to-vtt');
...
const pages = [
  ...
  },
  {
    ...
    content:`<section class="page-hero"><div class="container tools-intro"><p class="eyebrow">Tecnología VUMARI</p><h1>VUMARI Tools</h1><p class="lead">Herramientas digitales desarrolladas progresivamente por VUMARI STUDIOS.</p></div></section><section class="section section--soft"><div class="container tools-family"><div class="tools-family__heading"><p class="eyebrow">Primera familia</p><h2>${escape(toolsData.families[0].name)}</h2><p>Utilidades para formatos multimedia, comenzando por transformaciones ligeras y verificables.</p></div><div class="tools-grid">${toolsData.tools.map(tool => `<article class="tool-card"><p class="tool-card__status">${tool.status === 'experimental' ? 'Experimental' : escape(tool.status)}</p><h3>${escape(tool.shortName)}</h3><p>${escape(tool.summary)}</p><p class="tool-card__format">${escape(tool.input.formats.join(', ').toUpperCase())} <span aria-hidden="true">→</span> ${escape(tool.output.formats.join(', ').toUpperCase())}</p>${tool.id === 'media-srt-to-vtt' ? '<a href="srt-a-vtt/">Abrir herramienta experimental <span aria-hidden="true">→</span></a>' : ''}</article>`).join('')}</div><p class="tools-family__cta">¿Necesitas una solución digital para tu proyecto? <a href="../cotizacion.html">Solicita una cotización</a>.</p></div></section>`
  },
  {
    file:'herramientas/srt-a-vtt/index.html', canonicalPath:'herramientas/srt-a-vtt/', assetPrefix:toolPagePrefix, noindex:true,
    pageScript:'scripts/tools/tool-controller.js',
    title:srtToVttTool.seo.title,
    description:srtToVttTool.seo.description,
    content:`<section class="page-hero"><div class="container"><nav class="breadcrumb" aria-label="Ruta de navegación"><ol><li><a href="${withPrefix(toolPagePrefix, 'index.html')}">Inicio</a></li><li><a href="../">VUMARI Tools</a></li><li aria-current="page">SRT → VTT</li></ol></nav><p class="eyebrow">Experimental</p><h1>Convertir SRT a VTT</h1><p class="lead">Convierte subtítulos SubRip al formato WebVTT directamente en tu navegador.</p></div></section>
    <section class="section section--soft"><div class="container tool-detail-layout"><div><div class="tool-workspace" data-tool-controller data-processor="${escape(srtToVttTool.processor)}" data-state="idle" hidden><div><p class="eyebrow">Conversor local</p><h2>Selecciona tu archivo SRT</h2></div><p class="privacy-note">${escape(srtToVttTool.privacy.message)}</p><form class="tool-form" data-tool-form><label class="tool-file-label" for="srt-file">Archivo SRT</label><p id="srt-file-help">Se admite un archivo con extensión .srt y texto UTF-8.</p><input class="tool-file" id="srt-file" name="subtitle" type="file" accept=".srt,application/x-subrip,text/plain" aria-describedby="srt-file-help" data-tool-file><button class="button button--primary" type="submit" data-tool-submit disabled>Convertir a VTT</button><p class="tool-status" role="status" aria-live="polite" data-tool-status></p></form><section class="tool-result" tabindex="-1" data-tool-result hidden><h3>Resultado WebVTT</h3><pre data-tool-result-text></pre><a class="button button--secondary" href="#" data-tool-download hidden>Descargar VTT</a></section></div><noscript><p class="card">Esta herramienta necesita JavaScript para procesar el archivo localmente. La información y las instrucciones de esta página siguen disponibles.</p></noscript></div>
    <div class="tool-information"><article><h2>Cómo funciona</h2><ol><li>Selecciona un archivo SRT.</li><li>Revisa que el archivo sea válido.</li><li>Convierte y descarga el resultado VTT.</li></ol></article><article><h2>Formatos</h2><p><strong>Entrada:</strong> SRT con timestamps HH:MM:SS,mmm.</p><p><strong>Salida:</strong> WebVTT con timestamps HH:MM:SS.mmm.</p></article><article><h2>Limitaciones</h2><ul><li>Esta versión admite texto UTF-8 con o sin BOM.</li><li>No convierte archivos UTF-16, Windows-1252 ni otras codificaciones.</li><li>No corrige, traduce ni reescribe el texto.</li></ul></article><article><h2>Preguntas frecuentes</h2><h3>¿El archivo se sube a internet?</h3><p>No. La lectura y la conversión ocurren localmente en este navegador.</p><h3>¿Se conserva la numeración SRT?</h3><p>No es necesaria en WebVTT; se conserva el orden de los subtítulos.</p></article><article><h2>¿Necesitas una solución digital?</h2><p>VUMARI STUDIOS desarrolla sitios y herramientas orientados a necesidades concretas.</p><div class="actions"><a class="button button--secondary" href="${withPrefix(toolPagePrefix, 'servicios.html')}#desarrollo-web">Conoce desarrollo web</a><a class="button button--primary" href="${withPrefix(toolPagePrefix, 'cotizacion.html')}">Solicita una cotización</a></div></article></div></div></section>`
  }
];
